    ...
    public static void Postfix(List<GeyserGenericConfig.GeyserPrefabParams> __result)
    {
      try
      {
        ...
        if (File.Exists(configurationFilePath))
        {
          ...
          {
            var values = lines[index].Split(Separators, StringSplitOptions.RemoveEmptyEntries);
            ...
            types[identity] = new GeyserTypeConfig
            {
              ...
              Temperature               = float.Parse(values[2]),
              Pressure                  = float.Parse(values[3]),
              RateMultiplier            = float.Parse(values[4]),
              RatePerCycle              = new Range(float.Parse(values[05]), float.Parse(values[06])),
              ActivePeriod              = new Range(float.Parse(values[07]), float.Parse(values[08])),
              ActivePeriodPercentage    = new Range(float.Parse(values[09]), float.Parse(values[10])),
              EruptionPeriod            = new Range(float.Parse(values[11]), float.Parse(values[12])),
              EruptionPeriodPercentage  = new Range(float.Parse(values[13]), float.Parse(values[13]))
            };
          }
